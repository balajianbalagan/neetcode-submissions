class Solution {
    ...
    public boolean bfs(int[][] heights,int[][] ocean,int i,int j,int endi,int endj,int num,int[][] visited){
        //invalid oob
        if(i<0 || i>=heights.length || j<0 || j>=heights[0].length ||  ocean[i][j]==-1 || heights[i][j]>num || visited[i][j]==1){
            return false;
        }
     
        //base case
        if((ocean[i][j]==1 && heights[i][j]<=num)  || ((i==endi || j==endj) && heights[i][j]<=num)){
            ocean[i][j]=1;
            return true;
        }
        visited[i][j]=1;
    ...
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        ...
        for(int i=0;i<rows;i++){
            ...
            System.out.println("----I LOVE U AKSHU--------");
        }
        return arr;
    }
}
